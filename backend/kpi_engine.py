...
def compute_financial_kpis(
    ...
    trend_analysis = analista.calcola_trend_predittivo(
        nome_asset=asset_name, company_id=company_id
    )
    ...
    risk_level = (
        "ALTO" if risk_score > 7.0 else ("MEDIO" if risk_score > 5.0 else "BASSO")
    )
    ...
    return {
        ...
        "trend_momentum": {
            ...
            "valutazione": trend_analysis.get(
        ...
        "ai_strategic_action_plan": [
            {
                ...
                "action": trend_analysis.get(
                    "azione", "Monitorare l'andamento delle ore reali e dei flussi."
                ),
