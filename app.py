...
# ========================================================
# GESTIONE DELLE PAGINE (STRUTTURA UNIFICATA)
# ========================================================
...
if scelta == "🏠 Home":
    ...
    st.write("Pannello di controllo amministrativo globale delle metriche aggregate.")
    ...
    with st.sidebar:
        if is_admin:
            ...
            w1, w2 = 0.7, 0.3
            ritardo, f_stress = 0, 1.0

    # SAFETY FIX: Inizializzazione per prevenire i NameError
    report_analisi = None
    risultati_sim = None
    risultato_wr = None
            ...
            if lista_asset:
                ...
                if asset_a_rischio:
                    ...
                    try:
                        ...
                        mailer = EmailManager()
                        corpo_mail = f"⚠️ Alert: Rilevati {len(asset_a_rischio)} nodi critici nel comparto {reparto_scelto}."
    ...
    if report_analisi:
        ...
        consigli_settore = {
            "PRIMARIO_ALIMENTARE": {"ufficio": "Qualità e Logistica", "guadagno": "+15% riduzione sprechi", "prognosi": "Rischio deperibilità elevato. La velocità di rotazione è vitale."},
            "SECONDARIO_MANIFATTURA": {"ufficio": "Produzione e Acquisti", "guadagno": "+12% ottimizzazione stock", "prognosi": "Rallentamento flussi rilevato. Possibile fermo macchina tra 10gg."},
            "TERZIARIO_LOGISTICA": {"ufficio": "Ufficio Spedizioni / Traffico", "guadagno": "+20% efficienza consegne", "prognosi": "Collo di bottiglia nei vettori. Saturazione magazzino imminente."},
            "EDILE": {"ufficio": "Capocantiere / Approvvigionamento", "guadagno": "+10% gestione materiali", "prognosi": "Fluttuazione dei prezzi delle materie prime rilevata. Stringere i contratti di fornitura."},
            "GENERALE": {"ufficio": "Ufficio Operation / Management", "guadagno": "+10% efficienza standard", "prognosi": "Nessuna anomalia strutturale critica rilevata. Continuare il monitoraggio ordinario."}
        }
    ...
    st.write("Consultazione dei file Excel e CSV elaborati sul File System e nel Database.")
    
    # 1. Scansione File Fisici
    ...
    if lista_file_storico:
        ...
        st.info("📭 Al momento non sono presenti file CSV o Excel nelle cartelle fisiche.")
        
    st.markdown("---")
    
    # 2. Log del Database Aziendale (Accorpati correttamente)
    st.subheader("📊 Record Log Caricamenti (Database)")
    ...
    if not df_log.empty:
        ...
        if st.button("🗑️ Svuota tutti i dati"):
            ...
            st.success("Tutti i log storici e gli asset sono stati rimossi con successo dal database.")
            st.rerun()
    else:
        st.warning("📭 Nessun log di caricamento registrato nel database.")