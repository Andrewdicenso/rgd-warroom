...
class ConsulenteAziendale:
    ...
    def carica_configurazione(self):
        ...
        while not os.path.exists(self.file_config):
            print(
                f"--- ERRORE: File di configurazione '{self.file_config}' non trovato ---"
            )
    ...
    def analizza_cac(self, valore):
        if valore <= (self.soglia_cac * 0.8):
            stato, consiglio = "ECCELLENTE", "CAC ottimo, puoi scalare."
        elif valore <= self.soglia_cac:
            stato, consiglio = "SOSTENIBILE", "CAC in linea."
        else:
            stato, consiglio = "CRITICO", "CAC troppo alto. Rivedi i canali."
        self.ultima_analisi = {"kpi": "CAC", "stato": stato, "consiglio": consiglio}
        return stato, consiglio

    def analizza_ltv(self, valore):
        if valore >= (self.soglia_ltv * 1.2):
            stato, consiglio = "ECCELLENTE", "LTV molto alto."
        elif valore >= self.soglia_ltv:
            stato, consiglio = "SOSTENIBILE", "Il valore del cliente è sano."
        else:
            stato, consiglio = "CRITICO", "LTV basso. Migliora la fidelizzazione."
        self.ultima_analisi = {"kpi": "LTV", "stato": stato, "consiglio": consiglio}
        return stato, consiglio

    def analizza_burn_rate(self, valore):
        if valore <= (self.soglia_burn_rate * 0.7):
            stato, consiglio = "ECCELLENTE", "Ottima gestione cassa."
        elif valore <= self.soglia_burn_rate:
            stato, consiglio = "SOSTENIBILE", "Burn rate sotto controllo."
        else:
            stato, consiglio = "CRITICO", "Stai bruciando cassa troppo velocemente."
        self.ultima_analisi = {
            "kpi": "BURN_RATE",
            "stato": stato,
            "consiglio": consiglio,
    ...
    def analizza_margine_netto(self, valore):
        if valore >= self.soglia_margine:
            stato, consiglio = "ECCELLENTE", "Margine solido."
        else:
            stato, consiglio = "CRITICO", "Utile eroso. Controlla i costi."
        self.ultima_analisi = {
            "kpi": "MARGINE_NETTO",
            "stato": stato,
            "consiglio": consiglio,
    ...
    def analizza_tasso_conversione(self, valore):
        if valore >= self.soglia_conversione:
            stato, consiglio = "ECCELLENTE", "Conversione alta."
        else:
            stato, consiglio = "CRITICO", "Conversione bassa. Rivedi l'offerta."
        self.ultima_analisi = {
            "kpi": "TASSO_CONVERSIONE",
            "stato": stato,
            "consiglio": consiglio,
        }
        return stato, consiglio
