...
def analyze_file(file_id: str):
    ...
    try:
        with engine.connect() as connection:
            ...
            query = text("SELECT file_name, client_id FROM file_uploads WHERE id::text = :file_id;")
            ...
            if not file_path.exists():
                raise HTTPException(status_code=404, detail=f"File fisico non trovato in {file_path}")
            ...
            if risk_score > 7.5:
                momentum = kpi_results.get("trend_momentum", {}).get("momentum_perc", "0%")
                action_plan = kpi_results.get("ai_strategic_action_plan", [{}])[0].get("action", "Monitoraggio")
