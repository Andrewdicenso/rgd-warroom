...
def esegui_analisi_evolutiva(
    azienda_id, dati_file_completo, contesto_rilevato, storico_completo=None
):
    """
    ...
    try:
        ...
        print(
            f"🤖 [{nome_progetto} AI]: Avvio scansione olistica sul file completo di {contesto_rilevato}"
        )
        ...
        report_diagnostico = ""
        azioni_di_rimedio_marketing = (
            "Nessuna azione richiesta: i parametri aziendali sono stabili."
        )

        # Conversione dei dati in DataFrame Pandas (gestione olistica dell'intero foglio)
        df_attuale = pd.DataFrame(dati_file_completo)
        df_storico = (
            pd.DataFrame(storico_completo)
            if storico_completo is not None
            else pd.DataFrame()
        )
        ...
        if contesto_rilevato == "CONTABILITÀ":
            ...
            incassi_attuali = (
                df_attuale["incassi"].sum() if "incassi" in df_attuale.columns else 0
            )

            # Estrazione e calcolo della media storica dai dati del database
            if (
                not df_storico.empty
                and "contesto" in df_storico.columns
                and "incassi" in df_storico.columns
            ):
                df_storico_contabile = df_storico[
                    df_storico["contesto"] == "CONTABILITÀ"
                ]
                media_storica = (
                    df_storico_contabile["incassi"].mean()
                    if not df_storico_contabile.empty
                    else incassi_attuali
                )
            else:
            ...
            if incassi_attuali < (
                media_storica * regole_contabili["tolleranza_flessione_ricavi"]
            ):
                salute["CONTABILITÀ"] = (
                    round((incassi_attuali / media_storica) * 100)
                    if media_storica > 0
                    else 100
                )
                ...
                if (
                    "MAGAZZINO"
                    in regole_contabili["reparti_da_interrogare_su_anomalia"]
                ):
                    ...
                    if (
                        not df_storico.empty
                        and "contesto" in df_storico.columns
                        and "giorni_stoccaggio" in df_storico.columns
                    ):
                        df_magazzino_storico = df_storico[
                            df_storico["contesto"] == "MAGAZZINO"
                        ]
                        merci_bloccate = df_magazzino_storico[
                            df_magazzino_storico["giorni_stoccaggio"]
                            > regole_magazzino["limite_giorni_giacenza_critica"]
                        ]
                        ...
                        if not merci_bloccate.empty:
                            ...
                            canali_marketing = rgd_config[
                                "motore_prescrittivo_marketing"
                            ]["canali_azione_veloce"]
        ...
        # --- PILASTRO 2: MAGAZZINO ---
            ...
            if (
                "quantita" in df_attuale.columns
                and "costo_unitario" in df_attuale.columns
            ):
                valore_immobilizzato = (
                    df_attuale["quantita"] * df_attuale["costo_unitario"]
                ).sum()
            else:
            ...
            if (
                valore_immobilizzato
                > regole_magazzino["soglia_valore_sovraccarico_euro"]
            ):
                salute["MAGAZZINO"] = 50
                ...
                canali_marketing = rgd_config["motore_prescrittivo_marketing"][
                    "canali_azione_veloce"
                ]
        ...
        return {
            ...
            "diagnostica": (
                report_diagnostico
                if report_diagnostico
                else "Tutti i reparti analizzati sono in perfetto equilibrio operativo."
            ),
            "soluzione_ritrovata": azioni_di_rimedio_marketing,
            "impatto_futuro_stimato": (
                "Rischio erosione e contrazione della liquidità di cassa entro 30 giorni se non viene applicata la direttiva di marketing prescritta."
                if indice_solidita < 85
                else "I flussi finanziari e operativi stimati sul lungo termine non mostrano criticità latenti."
            ),
        }
