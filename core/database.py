...
class DatabaseAziendale:
    ...
    def get_utente_by_id(self, user_id: int):
        try:
            ...
            return {
                "id": row[0], 
                "email": email_dec.decode() if isinstance(email_dec, bytes) else email_dec, 
                "password_hash": row[2], 
                "ruolo": row[3], 
                "azienda": azienda_dec.decode() if isinstance(azienda_dec, bytes) else azienda_dec
            }
        except: 
            return None
    ...
    def supervisione_admin_metriche_globali(self):
        try:
            with self._get_conn() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT email, ruolo, azienda, data_creazione FROM utenti")
                    rows = cur.fetchall()
    ...
    def recupera_log_caricamenti_admin(self):
        try:
            with self._get_conn() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT * FROM log_caricamenti ORDER BY timestamp DESC")
                    rows = cur.fetchall()
                    colnames = [desc[0] for desc in cur.description]
    ...
    def registra_caricamento(self, user_id, contesto, nome_file):
        try:
            ...
            with self._get_conn() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "INSERT INTO log_caricamenti (user_id, azienda, contesto, nome_file) VALUES (%s, %s, %s, %s)", 
    ...
    def salva_asset(self, user_id, nome_asset, rischio, **kwargs):
        try:
            ...
            with self._get_conn() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "INSERT INTO asset_logs (user_id, company_id, nome, tipo, rischio, momentum, volatilita) VALUES (%s, %s, %s, %s, %s, %s, %s)",
    ...
    def calcola_e_salva_kpi_correnti(self, user_id):
        ...
        try:
            ...
            if not res_recent or res_recent[1] == 0: 
                return {"solidita": 100, "trend": "Stabile", "variazione": 0, "rischio_medio": 0}
            ...
            return {"solidita": 0, "trend": "Errore"}
    ...
    def recupera_attivita_globale(self):
        ...
        try:
            with self._get_conn() as conn:
                with conn.cursor() as cur:
                    # FIX: Allineato 'contesto' (corretto il context inserito per errore)
                    query = """
