...
class DatabaseAziendale:
    ...
    def crea_utente(self, email, password, ruolo="user", azienda=None):
        try:
            with self._get_conn() as conn:
                ...
                # Inserimento utente
                result = conn.execute(
                    text("INSERT INTO utenti (email, password_hash, ruolo, azienda) VALUES (:email, :password_hash, :ruolo, :azienda) RETURNING id"),
                    {"email": email_enc, "password_hash": password_hash, "ruolo": ruolo, "azienda": None}
                )
                ...
                conn.execute(
                    text("UPDATE utenti SET azienda = :azienda WHERE id = :id"),
                    {"azienda": azienda_enc, "id": user_id}
                )
    ...
    def get_utente_by_email(self, email):
        try:
            with self._get_conn() as conn:
                result = conn.execute(
                    text("SELECT id, email, password_hash, ruolo, azienda FROM utenti")
                )
    ...
    def get_utente_by_id(self, user_id: int):
        try:
            with self._get_conn() as conn:
                result = conn.execute(
                    text("SELECT id, email, password_hash, ruolo, azienda FROM utenti WHERE id = :id"),
                    {"id": user_id}
                )
                row = result.fetchone()
    ...
    def supervisione_admin_metriche_globali(self):
        try:
            ...
            logger.error(f"Errore supervisione_admin: {e}")
            return pd.DataFrame()
    ...
    def registra_caricamento(self, user_id, contesto, nome_file):
        try:
            ...
            with self._get_conn() as conn:
                conn.execute(
                    text("INSERT INTO log_caricamenti (user_id, azienda, contesto, nome_file) VALUES (:user_id, :azienda, :contesto, :nome_file)"),
                    {"user_id": user_id, "azienda": azienda, "contesto": contesto, "nome_file": nome_file}
                )
                conn.commit()
        except Exception as e:
    ...
    def salva_asset(self, user_id, nome_asset, rischio, **kwargs):
        try:
            ...
            with self._get_conn() as conn:
                conn.execute(
                    text("INSERT INTO asset_logs (user_id, company_id, nome, tipo, rischio, momentum, volatilita) VALUES (:user_id, :company_id, :nome, :tipo, :rischio, :momentum, :volatilita)"),
                    {
                        "user_id": user_id, 
                        "company_id": azienda, 
                        "nome": nome_asset, 
                        ...
                        "volatilita": kwargs.get('volatilita')
                    }
                )
                conn.commit()
        except Exception as e:
    ...
    def calcola_e_salva_kpi_correnti(self, user_id):
        ...
        try:
            with self._get_conn() as conn:
                result = conn.execute(
                    text("SELECT AVG(rischio) FROM asset_logs WHERE user_id = :user_id AND timestamp >= NOW() - INTERVAL '1 hour'"),
                    {"user_id": user_id}
                )
                rischio_medio = result.fetchone()[0]
            ...
            logger.error(f"Errore calcola_kpi: {e}")
