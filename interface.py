...
# 2. LOGICA WAR ROOM (Ingestione Documentale Standard)
    ...
    if file_caricato:
        ...
        if risultato['status'] == 'success':
            ...
            if lista_asset:
                # SALVATAGGIO AUTOMATICO IMMEDIATO
                conn = get_connection()
                cur = conn.cursor()
                for asset in lista_asset:
                    cur.execute("INSERT INTO asset_logs (nome, rischio, company_id, timestamp, eliminato) VALUES (%s, %s, %s, %s, False)", 
                                (asset['nome'], asset['rischio'], COMPANY_ID, datetime.now()))
                conn.commit()
                conn.close()
                st.success("✅ File caricato e dati salvati nel sistema!")
                st.rerun() 
